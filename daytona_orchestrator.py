...
from daytona_sdk import CreateSandboxBaseParams, Daytona, DaytonaConfig
...
class DaytonaOrchestrator:
    def __init__(self):
        ...
        self.target_repo = os.environ.get(
    ...
    def get_tool_specification(self):
        """Returns the Tool Specification prompt for the LLM Orchestrator."""
        return """### Available MCP Tools (Daytona Sandboxes)
- `resume_parser`: Converts raw PDF to structured YAML. Use ONLY for initial uploads.
- `ats_validator`: Checks for 'Parseability' and 'Keyword Density.' Use as a final verification.
- `tailor_engine`: Rewrites specific bullet points. Requires [JD] and [YAML_Block].

### Execution Rules
1. **Constraint First:** Never rewrite a bullet point unless you have retrieved 100% factual data from the RAG Document Server.
2. **Privacy Protocol:** If the user asks for personal data not in the current `resume.yaml`, you MUST call `memory_server.get_persona` instead of guessing.
3. **Loop Control:** If the `ats_validator` returns a score < 85, you are authorized to auto-correct the bullet points ONE time before asking for user feedback."""

    def analyze_ats(self, resume_text, job_desc_text, auto_correct=False, correction_callback=None):
        """
        1. Create Worker
        ...
        Implements Loop Control: If score < 85 and auto_correct is True, attempts to improve and re-run.
        """
        ...
        try:
            ...
            # Run Analysis
            cmd = "python ats_analyzer.py resume.txt job_desc.txt"
            print(f"Running command: {cmd}")
            response = sandbox.process.exec(cmd)
            print(f"Worker Output:\n{response.result}")
            ...
            score = result.get("score", 0)  # Assuming ats_analyzer returns a 'score' field
            if auto_correct and score < 85 and correction_callback:
                print(f"Score {score} < 85. Initiating Auto-Correction Loop (One-Time)...")
                # 1. Get suggestions/rewrite
                new_resume_text = correction_callback(resume_text, result, job_desc_text)

                if new_resume_text and new_resume_text != resume_text:
                    print("Re-running ATS Analysis on corrected resume...")
                    # Update file in sandbox
                    self.upload_file(sandbox, "resume.txt", new_resume_text)

                    # Re-run
                    response_retry = sandbox.process.exec(cmd)
                    if response_retry.exit_code == 0:
                        result_retry = json.loads(response_retry.result)
                        # Merge results or return new one
                        result_retry["original_score"] = score
                        result_retry["auto_corrected"] = True
                        return result_retry
